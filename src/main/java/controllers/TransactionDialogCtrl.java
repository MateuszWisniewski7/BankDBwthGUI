...
public class TransactionDialogCtrl {
    ...
    Label tip;
    @FXML
    ChoiceBox<CustomersTableModel> choiceBox;
    private final Pattern pattern = Pattern.compile("([-]?\\d*(\\.?\\d{0,2}))");
    private LocalDateTime dateTime = LocalDateTime.now();

    public TransactionsTableModel create(Integer customerId) {
        if (!(value.getText().indexOf('.')
              < 0)
            && !(Double.toString(Double.parseDouble(value.getText()))
                       .substring(Double.toString(Double.parseDouble(value.getText()))
                                        .indexOf('.'))
                       .length()
                 < 3))
            return new TransactionsTableModel(Timestamp.valueOf(dateTime),
                                              Double.toString(Double.parseDouble(value.getText())),
                                              customerId);
        else
            return new TransactionsTableModel(Timestamp.valueOf(dateTime),
