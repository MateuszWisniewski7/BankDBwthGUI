...
public class DataSource {
    ...
    private static final String CONNECTION_STRING =
            "jdbc:mysql://"
            + SERVER
            + "/?useSSL=false&useTimezone=true&serverTimezone=Europe/Warsaw";
    ...
    public boolean open() {
        try {
            ...
            String createDB = "CREATE DATABASE IF NOT EXISTS "
                              + DB_NAME;
            String createBranches = "CREATE TABLE IF NOT EXISTS "
                                    + TABLE_BRANCHES
                                    + " ("
                                    + COLUMN_BRANCH_ID
                                    ...
                                    + ")"
                                    + ")";
            String createCustomers = "CREATE TABLE IF NOT EXISTS "
                                     + TABLE_CUSTOMERS
                                     + " ("
                                     + COLUMN_CUSTOMER_ID
                                     + " int NOT NULL AUTO_INCREMENT, "
                                     ...
                                     + ")"
                                     + ")";
            String createTransactions = "CREATE TABLE IF NOT EXISTS "
                                        + TABLE_TRANSACTIONS
                                        + " ("
    ...
    public void addBranch(BranchesTableModel branch) {
        String insertBranch = "INSERT INTO "
                              + TABLE_BRANCHES
                              + " ("
                              + COLUMN_BRANCH_ID
                              + " , "
        ...
        try {
            ...
            System.out.println("ERROR in addBranch method: "
                               + e.getMessage());
        }
    }
    ...
    public void addCustomer(CustomersTableModel customer, double initial) {
        String insertCustomer = "INSERT INTO "
                                + TABLE_CUSTOMERS
                                + " ("
                                + COLUMN_CUSTOMER_ID
                                + " , "
        ...
        addTransaction(new TransactionsTableModel(java.sql.Timestamp.valueOf(LocalDateTime.now()),
                                                  Double.toString(initial),
                                                  customer
                                                          .getId()));
    ...
    public void addTransaction(TransactionsTableModel transaction) {
        ...
        try {
            ...
            System.out.println("ERROR in addTransaction method: "
                               + e.getMessage());
        }
    }

    public void deleteBranch(int id) {
        String deleteBranch = "DELETE FROM "
                              + TABLE_BRANCHES
                              + " WHERE "
                              + COLUMN_BRANCH_ID
                              + " = "
        ...
        try {
            ...
            System.out.println("ERROR in deleteBranch method: "
                               + e.getMessage());
        }
    ...
    public void deleteCustomer(int id) {
        ...
        try {
            ...
            System.out.println("ERROR in deleteCustomer method: "
                               + e.getMessage());
        }
...
//        String date = dateLocal.toString().replace('T',' ');
        ...
        String deleteTransaction = "DELETE FROM "
                                   + TABLE_TRANSACTIONS
                                   + " WHERE "
                                   + COLUMN_TRANSACTION_DATE
                                   + " = "
        ...
        try {
            ...
            System.out.println("ERROR in deleteTransaction method: "
                               + e.getMessage());
        }
    }
    ...
    public ObservableList<BranchesTableModel> loadBranches() {
        try {
            ...
            String selectBranches = "SELECT * FROM "
                                    + TABLE_BRANCHES;
            Statement statement = conn.createStatement();
            ResultSet resultSet = statement.executeQuery(selectBranches);
            while (resultSet.next()) {
                branchesObvList.add(new BranchesTableModel(resultSet.getInt(COLUMN_BRANCH_ID),
    ...
    public ObservableList<CustomersTableModel> loadCustomers(int branchId) {
        try {
            ...
            while (resultSet.next()) {
                customersObvList.add(new CustomersTableModel(resultSet.getInt(COLUMN_CUSTOMER_ID),
                                                             resultSet.getString(COLUMN_CUSTOMER_NAME),
                                                             false,
                                                             resultSet
                                                                     .getInt(COLUMN_CUSTOMER_BRANCH_ID)));
            }
    ...
    public ObservableList<CustomersTableModel> loadAllCustomers() {
        try {
            ObservableList<CustomersTableModel> customersObvList = FXCollections.observableArrayList();
            String selectCustomers = "SELECT * FROM "
                                     + TABLE_CUSTOMERS;
            Statement statement = conn.createStatement();
            ResultSet resultSet = statement.executeQuery(selectCustomers);
            while (resultSet.next()) {
    ...
    public ObservableList<TransactionsTableModel> loadTransactions(int customerId) {
        try {
            ...
            while (resultSet.next()) {
                transactionsObvList.add(new TransactionsTableModel(resultSet.getTimestamp(COLUMN_TRANSACTION_DATE),
                                                                   resultSet
                                                                           .getString(COLUMN_TRANSACTION_VALUE),
